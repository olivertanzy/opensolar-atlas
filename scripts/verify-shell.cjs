...
(async()=>{
 ...
 try{
  ...
  const cards=await page.locator('.model-card').evaluateAll(nodes=>nodes.map(n=>({w:n.getBoundingClientRect().width,h:n.querySelector('.model-art').getBoundingClientRect().height,button:n.querySelector('.primary-link').getBoundingClientRect().top})));
  assert(Math.abs(cards[0].w-cards[1].w)<1,'Subject cards share the same width');
  assert(Math.abs(cards[0].h-cards[1].h)<1,'Subject previews share the same aspect ratio');
  assert(Math.abs(cards[0].button-cards[1].button)<1,'Subject actions align despite different description lengths');
  await page.screenshot({path:path.join(output,'library-unified.png')});
